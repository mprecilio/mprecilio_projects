...
public class RestaurantActions {
	...
	private static void addFridge(Accounts currUser) {
		...
		int id = PersistJDBC.newFridge(currUser.getUsername());
		PersistData.fridgeList.add(new Fridge(id, currUser.getUsername()));
	}
	...
	private static String addFood(Accounts currUser) {
		...
		if (inUse == null) {
			...
			PersistJDBC.addFood(addFood, inUse.getUniqueId());
		}
	...
	private static String removeFood(Accounts currUser) {
		...
		if (inUse == null) {
			...
			if (inUse.getFood().size() > 0) {
				...
				int counter = 0;
				for(String temp : inUse.getFood()) {
					if (temp.toLowerCase().equals(removeFood.toLowerCase())) {
						counter++;
					}
				}
				for (int i = 0; i < inUse.getFood().size(); i++) {
					if (inUse.getFood().get(i).toLowerCase().equals(removeFood.toLowerCase())) {
						PersistJDBC.deleteFood(inUse.getFood().get(i), inUse.getUniqueId());
						inUse.getFood().remove(i);
						if(counter <= 1) return null;
					}
				}
				for(int i = 1; i < counter; i++) {
					PersistJDBC.addFood(removeFood, inUse.getUniqueId());
					if(i == counter - 1) return null;
				}
			} else {
	...
	private static String transferFood(Accounts currUser) {
		...
		addFoodView(currUser);
		String userIn = ValidInput.returnValid(5, 1);
		Fridge inUse = findFridge(userIn, currUser); //fridge 1
		if (inUse == null) {
			System.out.println(PrintMenus.ANSI_RED + "Invalid input! Returning to the menu." + PrintMenus.ANSI_RESET);
			return null;// invalid input
		} else {
			if (inUse.getFood().size() > 0) {
				...
				for (int i = 0; i < inUse.getFood().size(); i++) {
					if (inUse.getFood().get(i).toLowerCase().equals(transferFood.toLowerCase())) {
						transferFood = inUse.getFood().get(i);
						b = true;
						break;
					}
				}
				...
				Fridge inUse2 = findFridge(userIn2, currUser); // second fridge
				if (inUse2 == null) {
					System.out.println(
					...
					PersistJDBC.addFood(transferFood, inUse2.getUniqueId());
					//PersistJDBC.transferFood(transferFood, inUse.getUniqueId(), inUse2.getUniqueId());
					int counter = 0;
					for(String temp : inUse.getFood()) {
						if (temp.toLowerCase().equals(transferFood.toLowerCase())) {
							counter++;
						}
					}
					for (int i = 0; i < inUse.getFood().size(); i++) {
					...
					PersistJDBC.deleteFood(transferFood, inUse.getUniqueId());
					for(int i = 1; i < counter; i++) {
						PersistJDBC.addFood(transferFood, inUse.getUniqueId());
					}
				}
	...
	private static String removeFridge(Accounts currUser) {
		...
		if (inUse == null) {
			...
			PersistJDBC.deleteFridge(inUse.getUniqueId());
			...
			for (Fridge temp : PersistData.fridgeList) {
				if ((temp.getOwner().equals(currUser.getUsername())) && !("" + temp.getUniqueId()).equals(userIn))
					while (temp.getFood().size() < 3) {
						...
						PersistJDBC.addFood(inUse.getFood().get(0), temp.getUniqueId());
						PersistJDBC.deleteFood(inUse.getFood().get(0), inUse.getUniqueId());
...
//						}
						...
						PersistJDBC.newHealthInspectorFridge(temp.getUsername(), covertToInt(userIn2));
						System.out.println();
